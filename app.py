...
def resize_image_with_aspect_ratio(
    image: np.ndarray, min_size: int = 800, max_size: int = 1333
) -> np.ndarray:
    h, w = image.shape[:2]
    aspect_ratio = w / h
...
def inference(
    image, prompt: str, return_mask: bool = False, return_score: bool = False
) -> gr.Image:
    ...
    image_pil = visualize(
        image, results, return_mask=return_mask, draw_score=return_score
    )
    return image_pil
...
if __name__ == "__main__":
    with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue")) as demo:
        ...
        with gr.Row():
            gr.Examples(
                examples=[
                    ...
                    [
                        "asset/demo2.jpeg",
                        "wireless walkie-talkie . life jacket . atlantic cod . man . vehicle . accessory . cell phone .",
                    ],
                    ["asset/demo3.jpeg", "wine rack . bottle . basket"],
                    [
                        "asset/demo4.jpeg",
                        "Mosque. golden dome. smaller domes. minarets. arched windows. white facade. cars. electrical lines. streetlights. trees. pedestrians. blue sky. shadows",
                    ],
                    [
                        "asset/demo5.jpeg",
                        "stately building. columns. sculptures. Spanish flag. clouds. blue sky. street. taxis. van. city bus. traffic lights. street lamps. road markings. pedestrians. sidewalk. traffic sign. palm trees",
                    ],
                ],
        ...
        run.click(
            inference,
            inputs=[input_image, prompt, return_mask, return_score],
